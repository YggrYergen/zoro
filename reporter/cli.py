...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    if args.call_api:
        ...
        if args.no_diff:
            ...
            # Write diff outputs (both, for convenience)
            diff_md = output_dir / "current_dif.md"
            diff_alt = output_dir / "o.diff"
            try:
                with diff_md.open("w", encoding="utf-8") as f:
                    f.write(output_text or "")
                with diff_alt.open("w", encoding="utf-8") as f:
                    f.write(output_text or "")
                logger.info("Diff written to %s and %s", diff_md.resolve(), diff_alt.resolve())
            except Exception as exc:
